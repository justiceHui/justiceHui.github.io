...
### 풀이
무향 연결 그래프 $G(V, E)$에서 $\mathcal{I}_G = \left\{ I : I \subset E, E\setminus I\text{ can connects all vertex in } G \right\}$라고 정의하면 $\mathcal{M} = (S, \mathcal{I}_G)$는 Matroid입니다.

$\mathcal{I}_{G}' = \left\{ I : I \subset E, E\setminus I\text{ can connects all vertex in } G \text{ using edge colored with R or G} \right\}$, $\mathcal{I}_{G}'' = \left\{ I : I \subset E, E\setminus I\text{ can connects all vertex in } G \text{ using edge colored with G or B} \right\}$라고 정의합시다. $\mathcal{M}' = (S, \mathcal{I}_G')$와 $\mathcal{M}'' = (S, \mathcal{I}_G'')$ 역시 Matroid입니다.
